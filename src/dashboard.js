...
const Dashboard = () => {
  ...
  const placeBet = (gameId, betAmount) => {
    const game = games.find((g) => g.id === gameId);
    const lastPlayed = currentPlayer.lastPlayedGames[gameId] || 0;

    if (Date.now() - lastPlayed < game.cooldown) {
      setGameOutcome(
        `You need to wait ${Math.ceil(
          (game.cooldown - (Date.now() - lastPlayed)) / 1000
        )} seconds to play ${game.name} again.`
      );
      return false;
    }
    ...
    if (currentPlayer.chips < game.minBet) {
      setGameOutcome(
        `You don't have enough ${CURRENCY_NAME} to play. Minimum bet required: ${game.minBet}.`
      );
      return false;
    }
    ...
    if (
      ...
      betAmount > game.maxBet
    ) {
      setGameOutcome(
        `Invalid bet amount. Min: ${game.minBet}, Max: ${game.maxBet}`
      );
      return false;
    }
  ...
  const conductLottery = useCallback(() => {
    ...
    const winningTicket = lotteryTickets.find(
      (ticket) => ticket.number === winningNumber
    );

    if (winningTicket) {
      const winner = players.find(
        (player) => player.id === winningTicket.playerId
      );
      updatePlayerChips(winner.id, lotteryPool, 'Lottery win');
      setGameOutcome(
        `${winner.name} won the lottery prize of ${lotteryPool} ${CURRENCY_NAME}!`
      );
      setLotteryPool(1000);
    } else {
    ...
    setLotteryTickets([]);
    setNextLottery(
      new Date(new Date().setHours(20, 0, 0, 0) + 24 * 60 * 60 * 1000).getTime()
    );
    ...
    if (playerDoc.exists()) {
      ...
      achievements.forEach((achievement) => {
        if (!playerData.achievements.includes(achievement.id)) {
          if (
            achievement.id === 1 &&
            playerData.chips >= achievement.threshold
          ) {
            addAchievement(playerId, achievement.id);
          } else if (
            achievement.id === 2 &&
            playerData.bets.slice(-5).every((b) => b.chips > 0)
          ) {
            addAchievement(playerId, achievement.id);
          }
        }
  ...
  const addAchievement = async (playerId, achievementId) => {
    ...
    await setDoc(
      ...
      {
        achievements: [
          ...(players.find((p) => p.id === playerId)?.achievements || []),
          achievementId,
        ],
      },
      { merge: true }
  ...
  const handleSignUp = async (newUser) => {
    try {
      ...
      alert('Account created successfully! Welcome to the Club!');
  ...
  const updatePollOptions = (newOptions) => {
    ...
    setVotes({});
    setNextPollReset(
      new Date(new Date().setHours(24, 0, 0, 0) + 24 * 60 * 60 * 1000).getTime()
    );
  };
  ...
  if (showSignUp) {
    return (
      <SignUp onSignUp={handleSignUp} onCancel={() => setShowSignUp(false)} />
    );
  }
  ...
  return (
    <div className="dashboard">
      <header className="dashboard-header">
        <h1 className="dashboard-title">Casino Club Dashboard</h1>
      ...
      {isAdmin ? (
        <div className="admin-view">
          ...
          <div className="admin-panel">
                ...
                <tbody>
                  {players.map((player) => (
                    <tr key={player.id}>
                      ...
                      <td>{player.email}</td>
                      <td>
                        {player.chips} {CURRENCY_NAME}
                      </td>
                      <td>
                        <button onClick={() => handleSelectPlayer(player)}>
                          Select
                        </button>
                        <button onClick={() => resetPlayerChips(player.id)}>
                          Reset Chips
                        </button>
                        <button onClick={() => deletePlayer(player.id)}>
                          Delete
                        </button>
                        <button onClick={() => addMoreChips(player.id)}>
                          Add {STARTING_CHIPS} {CURRENCY_NAME}
                        </button>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="lottery-control">
              ...
              <p>
                Current Jackpot: {lotteryPool} {CURRENCY_NAME}
              </p>
              <button onClick={() => adjustLotteryPool(1000)}>
                Increase Jackpot
              </button>
              <button onClick={() => adjustLotteryPool(-1000)}>
                Decrease Jackpot
            ...
            <div className="poll-results">
              ...
              {pollOptions.map((option) => (
                <div key={option}>
                  {option}:{' '}
                  {Object.values(votes).filter((v) => v === option).length}
                </div>
              ))}
            </div>
            <div className="jackpot-control">
              ...
              <p>
                Current Jackpot: {jackpot} {CURRENCY_NAME}
              </p>
              <input
                type="number"
                value={jackpot}
        ...
        <div className="player-view">
          <div className="player-stats">
            ...
            <p>
              Current Balance: {currentPlayer.chips} {CURRENCY_NAME}
            </p>
            <p>
              Your Rank: {getPlayerRank(currentPlayer.id)} / {players.length}
            </p>
            <div className="action-buttons">
              ...
              <button onClick={buyLotteryTicket} className="lottery-button">
                Buy Lottery Ticket (100 {CURRENCY_NAME})
              </button>
            </div>
            {gameOutcome && <p className="game-outcome">{gameOutcome}</p>}
          </div>
              ...
              <div className="game-buttons">
                {games.map((game) => (
                  <button
                    ...
                    disabled={
                      Date.now() -
                        (currentPlayer.lastPlayedGames[game.id] || 0) <
                      game.cooldown
                    }
                    className="game-button"
                  >
                ...
                <div className="game-ui">
                  {selectedGame && selectedGame.id === 'blackjack' && (
                    <BlackjackGame
                      ...
                      placeBet={(amount) => placeBet(selectedGame.id, amount)}
                    />
                  )}
                  {selectedGame.id === 'poker' && (
                    <PokerGame
                      onClose={() => setShowGameModal(false)}
                      onBet={(amount) =>
                        updatePlayerChips(
            ...
            <div className="bets">
              ...
              <ul>
                {currentPlayer.bets
                  ...
                  .map((bet, index) => (
                    <li key={index}>
                      {bet.chips > 0 ? `+${bet.chips}` : bet.chips}{' '}
                      {CURRENCY_NAME}
          ...
          <div className="lottery-info">
            ...
            <p>
              Your Tickets:{' '}
              {
                lotteryTickets.filter(
                  (ticket) => ticket.playerId === currentPlayer.id
                ).length
              }
            </p>
            {lotteryTickets.filter(
              ...
              <button onClick={() => setShowTicketsModal(true)}>
                View Tickets
              </button>
            )}
          </div>
          ...
          {showTicketsModal && (
            <div className="modal">
              <div className="modal-content">
                ...
                <button onClick={() => setShowTicketsModal(false)}>
                  Close
                </button>
              </div>
            </div>
          ...
          <div className="leaderboard">
            <h3>Casino Leaderboard</h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                data={players.sort((a, b) => b.chips - a.chips).slice(0, 10)}
              >
